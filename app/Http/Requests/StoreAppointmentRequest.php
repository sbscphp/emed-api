...
class StoreAppointmentRequest extends FormRequest
{
    ...
    {
        return [
            'patient_id' => ['required', 'integer', 'exists:tenant.patients,id'],
            'visit_id' => ['nullable', 'integer', 'exists:tenant.patient_visits,id'],
            'appointment_type' => ['required', 'string', Rule::in(Appointment::TYPES)],
            'department_id' => ['required', 'integer', 'exists:tenant.departments,id'],
            'doctor_id' => ['required', 'integer', 'exists:landlord.users,id'],
            'visit_type' => ['required', 'string', Rule::in(Appointment::VISIT_TYPES)],
            'date' => ['required', 'date'],
    ...
    {
        return [
            'patient_id.required' => 'Please select a patient for the appointment.',
