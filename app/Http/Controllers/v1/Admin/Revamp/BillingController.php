...
use App\Models\User;
use App\Services\Revamp\BillingService;
use Throwable;
...
class BillingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $overview = $this->billingService->overview($request);

            $stats = $this->billingService->stats($request);
            $records = [
                ...$stats,
                'data' => $overview
            ];
            ...
            if ($request->has('export') && !empty($request->query('export'))) {
                ...
                return $this->billingService->export($overview, $format);
            }
    ...
    public function invoice(Request $request)
    {
        ...
        try {
            $overview = $this->billingService->invoiceOverview($request);

            if ($request->has('export') && !empty($request->query('export'))) {
                $format = $request->query('export');
                return $this->billingService->invoiceExport($overview, $format);
            }
    ...
    public function viewInvoice($id)
    {
        ...
        try {
            ...
            $billingInvoice = BillingLog::with(['service', 'billingLogDetails.treatment', 'billingLogDetails.labInvestigation', 'billingLogDetails.radiologyInvestigation', 'billingLogDetails.serviceUnit'])->find($id);
            if (!$billingInvoice) {
    ...
    public function summary(Request $request)
    {
        ...
        try {
            $overview = $this->billingService->summaryOverview($request);

            if ($request->has('export') && !empty($request->query('export'))) {
                $format = $request->query('export');
                return $this->billingService->summaryExport($overview, $format);
            }
    ...
    public function service(Request $request)
    {
        ...
        try {
            $overview = $this->billingService->serviceOverview($request);

            $stats = $this->billingService->serviceStats($request);
            $records = [
                ...$stats,
                'data' => $overview
            ];
            ...
            if ($request->has('export') && !empty($request->query('export'))) {
                $format = $request->query('export');
                return $this->billingService->serviceExport($overview, $format);
            }
    ...
    public function makePayment(Request $request)
    {
        try {
            ...
            $billing = BillingLog::with([
                ...
                ->find($request->billing_id);

            if (!$billing) {
                return JsonResponser::send(true, 'Record not found.', null, 200);
            }

            $record = $this->billingService->makePayment($request, $billing);
    ...
    public function updateResult(Request $request, $id)
    {
        try {
            ...
            $record = $this->billingService->updateResult($request, $test);
    ...
    public function updateTestStatus(Request $request, $id)
    {
        try {
            ...
            $record = $this->billingService->updateTest($request, $test);
