...
use App\Models\User;
use App\Services\Revamp\RadiologyService;
use Throwable;
...
class RadiologyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $overview = $this->radiologyService->overview($request);

            $stats = $this->radiologyService->stats($request);
            $records = [
                ...$stats,
                'data' => $overview
            ];
            ...
            if (isset($request->export)) {
                ...
                return $this->radiologyService->export($overview, $format);
            }
    ...
    public function updateResult(Request $request, $id)
    {
        try {
            DB::beginTransaction();
            $test = Radiology::with(['billingLogDetail'])->find($id);
            if (!$test) {
                return JsonResponser::send(true, 'Radiology test not found.', [], 404);
            }
    ...
    public function updateTestStatus(Request $request, $id)
    {
        try {
            ...
            $record = $this->radiologyService->updateTest($request, $test);
