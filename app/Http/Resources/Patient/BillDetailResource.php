 ...
 * without a second call. A settled bill also carries the card for the receipt
...
class BillDetailResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            // The card under "Download Receipt": what the file is called, how
            // big it is, how many pages it runs to. Null until the bill is
            // settled, which is also when /receipt starts answering with one.
            'can_download_receipt' => $isSettled,
            'receipt' => $isSettled ? ($this->receipt ?: null) : null,
            ...
            //
            // `amount` is this bill's share of the payment, not the charge — a
            // "pay all" checkout is worth more than any one invoice it cleared,
            // and printing the whole 85,000 here would not add up to the bill.
            // `charge_amount` is what the payer was actually debited, so the
            // reference can still be matched against their bank statement.
            'payments' => collect($this->payments ?: [])->map(fn($payment) => [
                'id' => $payment->id,
                'reference' => $payment->reference,
                'amount' => round((float) ($payment->applied_amount ?? $payment->amount), 2),
                'charge_amount' => round((float) $payment->amount, 2),
