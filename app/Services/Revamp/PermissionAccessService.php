...
{
    /**
     * Get access details for a permission scoped to a tenant.
     */
    public function getAccessDetails(User $user, Permission $permission, Tenant $tenant): array
    {
        $accessType = PermissionAccessTypeEnum::NONE->value;
        ...
        // Get role IDs for the user in the tenant (tenant DB)
        $tenantRoleIds = DB::connection('tenant')
            ->table('roles')
            ->join('role_user', 'roles.id', '=', 'role_user.role_id')
            ->where('role_user.user_id', $user->id)
        ...
        // Check if the permission is granted via these roles
        $userHasPermission = DB::connection('tenant')
            ->table('permissions')
            ->join('permission_role', 'permissions.id', '=', 'permission_role.permission_id')
            ->whereIn('permission_role.role_id', $tenantRoleIds)
        ...
        return [
            'access_type' => PermissionAccessTypeEnum::FULL->value,
            'remarks' => null,
        ];
    }
     ...
     */
    public function allPermissions(Tenant $tenant)
    {
        $user = Auth::user();

        // Fetch as Eloquent models on tenant connection
        $permissions = Permission::on('tenant')->get();

        return $permissions->map(function ($permission) use ($user, $tenant) {
            $accessDetails = $this->getAccessDetails($user, $permission, $tenant);
     ...
     */
    public function accessTo(User $user, string $permissionName, Tenant $tenant): bool
    {
        // Get user role IDs in tenant DB
        $tenantRoleIds = DB::connection('tenant')
            ->table('roles')
            ->join('role_user', 'roles.id', '=', 'role_user.role_id')
            ->where('role_user.user_id', $user->id)
        ...
        if ($tenantRoleIds->isEmpty()) return false;
